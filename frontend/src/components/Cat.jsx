          ...
          return (
            <div key={index} className="singleCol">
              <div className="catHeadingDiv">
                <div
                  to="/catagoryView"
                  className="catHeadingText"
                  style={{ textDecoration: "none" }}
                >
                  <span>{heading}</span>
                </div>
              </div>
              {mainAndSubCategories[index + 1].map((category, index) => (
                <SubCat key={index} name={category} />
              ))}
            </div>
          );
        })}
        ...
        <div className="insideCat1">
          <div className="allCat">
            ...
            <div className="SomeCat">
              {headerCategoryList.map((x, index) => (
                <SingleCat key={index} name={x} />
              ))}
            </div>
